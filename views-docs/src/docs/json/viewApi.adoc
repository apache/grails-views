...
All JSON views subclass the link:groovydoc/grails/plugin/json/view/JsonViewTemplate.html[JsonViewTemplate] class by default.

The `JsonViewTemplate` superclass implements the link:groovydoc/grails/plugin/json/view/api/JsonView.html[JsonView] trait which in turn extends the the link:groovydoc/grails/views/api/GrailsView.html[GrailsView] trait.
...
To customize content types and headers use the `response` object defined by the link:groovydoc/grails/views/api/HttpView.html[HttpView] trait:
...
Various aspects of the request can be accessed by the `request` object defined by the link:groovydoc/grails/views/api/HttpView.html[HttpView] trait:
...
Parameters can be accessed via the `params` object which is an instance of link:groovydoc/grails/views/api/http/Parameters.html[Parameters]:
