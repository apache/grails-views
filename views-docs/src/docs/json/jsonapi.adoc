...
The JSON views plugin for Grails provides JSON API support out-of-the-box. All JSON views have access to the `jsonapi` instance which implements link:groovydoc/grails/plugin/json/view/api/JsonApiViewHelper.html[JsonApiViewHelper].
...
Grails provides a way to customize the rendering of your domain class identifiers. To override the default behavior, register a bean that implements link:groovydoc/grails/plugin/json/view/api/jsonapi/JsonApiIdRenderStrategy.html[JsonApiIdRenderStrategy].
